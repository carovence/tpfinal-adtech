...
# Convertir la lista a DataFrame primero, después to_csv
pd.DataFrame(active_advertisers, columns=['advertiser_id']).to_csv(
    'data/advertiser_ids.csv', 
    index=False
)
...
possible_dates = [f'2026-04-{day:02d}' for day in range(18,31)] + [f'2026-05-{day:02d}' for day in range(1,30)]
...
import os
os.makedirs('data', exist_ok=True)
df_ads_views.to_csv('data/ads_views.csv', index=False)
